...
This is the only scanner on the host side. dgb.py install copies the launcher
and then calls it, so discovery and index generation exist in one place.
SCAN.COM mirrors it for the DOS machine; tests/test_scan_com.py holds them
to identical output.
...
The games root is never assumed — you always say where it is:
    ...
    python dgb.py scan --games-root ~/games --launcher-dir ~/image/DGB
...
def write_game_txt(path: Path, meta: dict[str, str], dry_run: bool = False) -> None:
    lines = ["# DOS Game Browser metadata - edit freely, then re-run the scan", ""]
...
def build_index(records: list[Record], sort: str, headers: bool) -> list[str]:
    ...
    lines = [
        "# GAMES.LST - generated by dgb.py scan",
...
def run(args: argparse.Namespace) -> int:
    ...
    # whenever the games root is nested under the launcher directory.
    exclude = None
